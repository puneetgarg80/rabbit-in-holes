...
import { RefreshCw, Trophy, Info, Minus, Plus, X, Play, SkipBack, SkipForward, ChevronLeft, ChevronRight, Pause, ClipboardList, Lightbulb } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'log' | 'assistant' | null>(null);
  ...
  return (
    <div className="h-[100dvh] bg-stone-50 text-stone-800 flex flex-col overflow-hidden relative font-sans">
      ...
      <div className="flex-none bg-white border-t border-stone-200 px-6 py-2 pb-4 safe-area-bottom z-20 grid grid-cols-2 gap-8 shadow-[0_-5px_20px_-5px_rgba(0,0,0,0.03)]">
         <button 
           onClick={() => setActiveTab(activeTab === 'log' ? null : 'log')}
           className={`flex flex-col items-center justify-center gap-1.5 py-2 rounded-2xl transition-all duration-300 ${activeTab === 'log' ? 'bg-stone-100 text-stone-900 -translate-y-1' : 'text-stone-400 hover:text-stone-600'}`}
         >
         ...
         <button 
           ...
           className={`flex flex-col items-center justify-center gap-1.5 py-2 rounded-2xl transition-all duration-300 ${activeTab === 'assistant' ? 'bg-indigo-50 text-indigo-600 -translate-y-1' : 'text-stone-400 hover:text-indigo-500'}`}
         >
           <Lightbulb className={`w-6 h-6 ${activeTab === 'assistant' ? 'fill-current stroke-2' : 'stroke-1.5'}`} />
           ...
           <div className="absolute bottom-0 left-0 right-0 z-40 bg-white rounded-t-[2rem] shadow-[0_-10px_40px_-10px_rgba(0,0,0,0.1)] animate-in slide-in-from-bottom duration-300 flex flex-col h-[55vh] max-h-[600px]">
              ...
              <div className="flex-none px-6 py-3 border-b border-stone-50 flex items-center justify-between">
                  <h3 className="text-lg font-bold text-stone-800 flex items-center gap-2">
                    {activeTab === 'log' ? <><ClipboardList className="w-5 h-5 text-stone-500"/> Investigation Log</> : <><Lightbulb className="w-5 h-5 text-indigo-500 fill-current"/> Wise Owl Assistant</>}
                  </h3>
