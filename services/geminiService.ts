...
export const getGameHint = async (
  ...
  const prompt = `
    You are a logic puzzle expert helping a player solve the "Quantum Cat and Mouse" puzzle.
    
    **Game Mechanics (Superposition):**
    1. There are ${holeCount} holes.
    2. The mouse is NOT in a single hole. It exists in a "superposition" of ALL possible holes consistent with the history of checks.
    3. Initially (Day 1), the mouse could be in ANY hole.
    ...
    5. AFTER the check, all remaining possible mice move to adjacent holes (left or right).
    ...
    **Task:**
    ...
    Suggest a move that efficiently reduces the number of possible mouse locations.
